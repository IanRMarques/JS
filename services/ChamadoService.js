...
export class ChamadoService {
  ...
  async dashboard() {
    const chamados = await this.chamadoRepository.listarTodos();
    const concluidos = chamados.filter((chamado) => {
      return chamado.resolvidoEm;
    });

    const somaMs = concluidos.reduce((soma, chamado) => {
      return soma + (chamado.resolvidoEm - chamado.criadoEm);
    }, 0); // 0 sero aqui serve para o primeiro item da lista, para não bugar o reduce

    const mediaHoras = concluidos.length > 0 ? somaMs / concluidos.length / (1000 * 60 * 60) : null; 
    ...
    return { porStatus: contagem, porCategoria: contagemCategoria, tempoMedioHoras: mediaHoras };
  }
}
